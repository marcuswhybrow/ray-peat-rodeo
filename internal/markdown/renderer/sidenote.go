...
func (t *SidenoteHTMLRenderer) renderSidenote(w util.BufWriter, source []byte, node gast.Node, entering bool) (gast.WalkStatus, error) {
	if entering {
      ...
      ></label><span
        ...
        class="z-10 block bg-white rounded-md shadow w-1/2 mr-[-5%] sm:mr-[-10%] md:mr-[-15%] lg:mr-[-25%] float-right clear-right text-sm relative p-4 before:content-[counter(sidenote)_'.'] before:float-left m-2 before:mr-1 before:text-gray-500 leading-5 align-middle transition-all [&>img]:py-2 last:[&>img]:pb-0"
      >
    `)
		if err != nil {
